 ...
 *
 */
namespace BardisCMS\RecipeBundle\Controller;
...
class DefaultController extends Controller
{
    ...
    // Get the Recipe page id based on alias from route
    public function aliasAction($alias, $extraParams = null, $currentpage = 0, $totalpageitems = 0) 
    {

        $page = $this->getDoctrine()->getRepository('RecipeBundle:Recipe')->findOneByAlias($alias);
    ...
    {   
        ...
        // Get data to display
        $page       = $this->getDoctrine()->getRepository('RecipeBundle:Recipe')->find($id);        
    ...
    public function getRequestedFilters($extraParams)
    {
        ...
        if (isset($extraParams[0]))
        {
            ...
            else
            {
                ...
                foreach($tags as $tag)
                {
                    $selectedTagObject = $this->getDoctrine()->getRepository('RecipeBundle:RecipeTag')->findOneByTitle(urldecode($tag));
        ...
        if (isset($extraParams[1]))
        {
            ...
            else
            {
                ...
                {
                    $selectedCategories[] = $this->getDoctrine()->getRepository('RecipeBundle:RecipeCategory')->findOneByTitle(urldecode($category));
                }
            }
        }
        else
    ...
    public function getCategoryFilterIds($selectedCategoriesArray)
    {
        ...
        {
            $selectedCategoriesArray = $this->getDoctrine()->getRepository('RecipeBundle:RecipeCategory')->findAll();
        }
    ...
    {       
        ...
        {
            $selectedTagsArray = $this->getDoctrine()->getRepository('RecipeBundle:RecipeTag')->findAll();
        }
    ...
    public function renderPage($page, $id, $publishStates, $extraParams, $currentpage, $totalpageitems, $linkUrlParams)
    {
        ...
        if ($page->getPagetype() == 'recipe_cat_page')
        {            
            $tagIds         = $page->getTags()->toArray();           
            $categoryIds    = $page->getCategories()->toArray();
            
            $filterForm     = $this->createForm('recipefiltersform');               
            ...
            {                
                $pageList = $this->getDoctrine()->getRepository('RecipeBundle:Recipe')->getTaggedCategoryItems($categoryIds, $id, $publishStates, $currentpage, $totalpageitems, $tagIds);                
            }
            if(!empty($tagIds) && empty($categoryIds))
            {                
                $pageList = $this->getDoctrine()->getRepository('RecipeBundle:Recipe')->getTaggedItems($tagIds, $id, $publishStates, $currentpage, $totalpageitems);                
            }
            else
            {
                $pageList = $this->getDoctrine()->getRepository('RecipeBundle:Recipe')->getCategoryItems($categoryIds, $id, $publishStates, $currentpage, $totalpageitems);
            }
            
            $pages      = $pageList['pages'];
            $totalPages = $pageList['totalPages'];
            
            return $this->render('RecipeBundle:Default:page.html.twig', array('page' => $page, 'pages' => $pages, 'totalPages' => $totalPages, 'extraParams' => $extraParams, 'currentpage' => $currentpage, 'linkUrlParams' => $linkUrlParams, 'totalpageitems' => $totalpageitems, 'filterForm' => $filterForm->createView()));
        }      
        else if ($page->getPagetype() == 'recipe_filtered_list')
        {               
            $filterForm     = $this->createForm('recipefiltersform');                
            $filterData     = $this->getRequestedFilters($extraParams);
        ...
        else if ($page->getPagetype() == 'recipe_home')
        {            
            ...
            $pageList = $this->getDoctrine()->getRepository('RecipeBundle:Recipe')->getAllItems($id, $publishStates, $currentpage, $totalpageitems);
        ...
        else if ($page->getPagetype() == 'recipe_article')
        {
            $productId      = $page->getProduct();            
            $relatedRecipes = $this->getDoctrine()->getRepository('RecipeBundle:Recipe')->getRelatedRecipes($productId, $id, $publishStates);
            
            return $this->render('RecipeBundle:Default:page.html.twig', array('page' => $page, 'relatedrecipes' => $relatedRecipes));
            
        }
        
        return $this->render('RecipeBundle:Default:page.html.twig', array('page' => $page));
    }
